...
class MessageFacadeIntegrationSpec extends IntegrationSpec implements SampleMessages {
  ...
  def "method 'findAllByConversationId' should return empty set if there are no messages belong to that id"() {
    when: "user tries to fetch all messages"
      def messages = this.messageFacade.findAllByConversationId(ObjectId.get())
    then:
      messages.size() == 0
  }
  ...
  def "method 'findAllByConversationId' should do nothing if there are messages belong to that id"() {
    ...
    when: "user tries to fetch all messages"
      def messages = this.messageFacade.findAllByConversationId(message.get("conversationId"))
    then:
      messages.size() == 1
  }
